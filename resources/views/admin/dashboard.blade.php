...
<div class="max-w-6xl mx-auto">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-2 sm:gap-3 mb-6 sm:mb-8">
        ...
        <a href="{{ route('admin.export.excel', ['date_from' => today()->subDays(14)->format('Y-m-d'), 'date_to' => today()->format('Y-m-d')]) }}"
        ...
        <a href="{{ route('admin.export.pdf', ['date_from' => today()->subDays(14)->format('Y-m-d'), 'date_to' => today()->format('Y-m-d')]) }}"
            class="flex items-center gap-3 bg-white border border-slate-200 rounded-xl px-4 py-3 hover:shadow-md transition-all group">
            ...
            <span class="text-sm font-semibold text-slate-700">Ekspor PDF</span>
        </a>
    </div>
