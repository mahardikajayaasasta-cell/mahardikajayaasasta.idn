...
<div class="max-w-6xl mx-auto">
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
        <div>
            <h2 class="text-xl sm:text-2xl font-bold text-slate-800">Manajemen Karyawan</h2>
            <p class="text-slate-500 text-xs sm:text-sm mt-1">Kelola data karyawan, akun, dan departemen</p>
        </div>
                    ...
                    <tr class="hover:bg-slate-50/50 transition-colors">
                        ...
                        <td class="px-5 py-4 text-right">
                            <div class="flex items-center justify-end gap-2">
                                ...
                                @if($user->is_active)
                                <form action="{{ route('admin.karyawan.destroy', $user) }}" method="POST" class="delete-form" data-title="Nonaktifkan Karyawan?" data-confirm="Akun {{ $user->name }} tidak akan bisa login atau melakukan absensi lagi.">
                                    @csrf
                                    @method('DELETE')
