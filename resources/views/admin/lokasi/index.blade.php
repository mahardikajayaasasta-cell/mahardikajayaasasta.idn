...
<div class="max-w-6xl mx-auto">
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
        <div>
            <h2 class="text-xl sm:text-2xl font-bold text-slate-800">Manajemen Lokasi Kerja</h2>
            <p class="text-slate-500 text-xs sm:text-sm mt-1">Kelola titik koordinat dan pengaturan jam kerja kantor/cabang</p>
        </div>
                    ...
                    <tr class="hover:bg-slate-50/50 transition-colors">
                        ...
                        <td class="px-5 py-4 text-right">
                            <div class="flex items-center justify-end gap-2">
                                ...
                                @if($loc->is_active)
                                <form action="{{ route('admin.lokasi.destroy', $loc) }}" method="POST" class="delete-form" data-title="Nonaktifkan Lokasi?" data-confirm="Lokasi {{ $loc->name }} tidak akan bisa digunakan lagi untuk absensi.">
                                    @csrf
                                    @method('DELETE')
